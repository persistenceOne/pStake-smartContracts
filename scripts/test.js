        ...
        if(action === "stake" || action === "unstake"){
            ...
            if(action === "stake"){
                ...
                const txData = await LiquidStakingInstance.methods
                    .stake(testData.pBridgeTestData[k].user.toString(), amount).encodeABI();
            ...
            const signedRawTx = await hdwallet.derive(`m/44'/60'/0'/0/0`).signTransaction({
                from: testData.pBridgeTestData[k].user.toString(),
                to: STokensArtifact.address,
                value: '0x0',
                data: txData
            })
            console.log("signedRawTx for calculateRewards: ", `0x${signedRawTx.toString('hex')}`)
            let sendTx = await web3.eth.sendSignedTransaction(signedRawTx);
            console.log("sendTx for calculateRewards: ", `0x${sendTx.toString('hex')}`)
        }
    }
}
