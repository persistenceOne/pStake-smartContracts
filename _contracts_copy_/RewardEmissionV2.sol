...
import "./interfaces/IRewardEmissionV2.sol";

contract RewardEmissionV2 is
	IRewardEmissionV2,
	PausableUpgradeable,
	AccessControlUpgradeable
{
	...
	function setWhitelistedAddress(
		...
		holderAddressLocal = holderContractAddress == address(0)
			? holderAddressLocal
			: holderContractAddress;

		uint256 holderWhitelistsLength = _holderWhitelists[holderAddressLocal]
			.length;
		...
		if (_stkContractAddresses[whitelistedAddress].length == 0) {
			...
			// save the sTokenAddresses to a local emit variable
			sTokenAddressesLocal = sTokenAddresses;
		}
		...
		returns (bool success)
	{
		...
		address holderContractAddressLocal = _holderContractAddress[
			whitelistedAddress
		];
		address lpContractAddressLocal = _lpContractAddress[whitelistedAddress];
		address[] memory stkContractAddresses = _stkContractAddresses[
			whitelistedAddress
		];

		uint256 lastHolderRewardTimestamp = _lastHolderRewardTimestamp[
			whitelistedAddress
		];
		...
		uint256 holderWhitelistsLength = _holderWhitelists[
			holderContractAddressLocal
		].length;
		for (j = 0; j < holderWhitelistsLength; j = j.add(1)) {
			if (
				_holderWhitelists[holderContractAddressLocal][j] ==
				whitelistedAddress
			) {
				whitelistedAddressExists = true;
				break;
		...
		if (whitelistedAddressExists) {
			// if the index of whitelistedAddress is the last one then just pop array
			if (j == holderWhitelistsLength.sub(1)) {
				_holderWhitelists[holderContractAddressLocal].pop();
			} else {
				// else replace the value in the index location with the value in the last index then pop array
				_holderWhitelists[holderContractAddressLocal][
					j
				] = _holderWhitelists[holderContractAddressLocal][
					holderWhitelistsLength.sub(1)
				];
