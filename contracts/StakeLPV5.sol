...
import "./interfaces/IStakeLPV2.sol";
import "./libraries/FullMath.sol";
import "./interfaces/IWhitelistedPTokenEmissionV3.sol";
import "./interfaces/IWhitelistedRewardEmissionV3.sol";

contract StakeLPV5 is
	IStakeLPV2,
	PausableUpgradeable,
	...
	ReentrancyGuardUpgradeable
{
		...
		)
	{
		...
		(
			...
			,
			lpTokenAddress
		) = IWhitelistedPTokenEmissionV3(_whitelistedPTokenEmissionContract)
		...
		) = IWhitelistedRewardEmissionV3(_whitelistedRewardEmissionContract)
		...
		returns (uint256[] memory rewardAmounts, address[] memory rewardTokens)
	{
		...
		for (i = 0; i < sTokenAddresses.length; i = i.add(1)) {
			rewardTokens[i] = ISTokensV3(sTokenAddresses[i]).getUTokenAddress();
			if (totalSupplyLPTimeShare > 0) {
				// calculated the updated rewardPool
				rewardPool = IUTokensV3(rewardTokens[i]).balanceOf(
					holderAddress
				);
