...
AccessControlUpgradeable
{
    ...
    //add pause for unstake function
    bool private _ifPaused;

    // MigrationAdmin contract address
    address public override _migrationAdminContract;

    //modifier to check only admin can call this function
    modifier onlyAdmin {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "LiquidStaking: Only admin can call this");
        _;
    }

    //modifier to check only pauser admin can call this function
    modifier onlyPauserAdmin {
        require(hasRole(PAUSER_ROLE, _msgSender()), "LiquidStaking: Only admin can call this");
        _;
    }
    ...
    returns (bool success)
    {
        ...
        require(
            ...
            "LiquidStaking: Range check error for fees"
        );
        _stakeFee = stakeFee;
    ...
    returns (bool success)
    {
        require(minStake >= 1, "LiquidStaking: Required minimum staked value");
        require(minUnstake >= 1, "LiquidStaking: Required minimum unstaked value");
        // check the anchor integration staking operation for rewards liquidation before you set _minStake
        _minStake = minStake;
    ...
    function setUnstakeEpoch(
        ...
        uint256 unstakeEpochPrevious,
        uint256 epochInterval
    ) public virtual override onlyAdmin returns (bool success) {
        require(unstakeEpochPrevious <= unstakeEpoch, "LiquidStaking: UnstakeEpochPrevious check error");
        if (unstakeEpoch == 0 && epochInterval != 0) revert("LiquidStaking: Epoch check error");
	 ...
	 *
	 */
    function setMigrationAdminContract(address migrationAdminContract)
    public
    virtual
    override
    onlyAdmin
    {
        _migrationAdminContract = migrationAdminContract;
        emit SetMigrationAdminContract(migrationAdminContract);
    }
    ...
    function _stakeDirect(
        ...
        require(
            ...
            "LiquidStaking: Amount should be non zero"
        );
    ...
    returns (uint256 amountStaked, uint256 amountWrapped)
    {
        ...
        require(hasRole(BRIDGE_ADMIN_ROLE, _msgSender()), "LiquidStaking: Unauthorised user");
    ...
    )
    {
        ...
        require(
            ...
            "LiquidStaking: Array length mismatch"
        );
    ...
    returns (uint256 amountStaked)
    {
        ...
        require(to == _msgSender(), "LiquidStaking: Unauthorised user");
        ...
        require(amount >= _minStake, "LiquidStaking: Invalid staked amount");
        ...
        require(currentUTokenBalance >= amount, "LiquidStaking: Balance should be greater than or equal to staked amount");
    ...
    returns (uint256 amountUnstaked)
    {
        ...
        require(_ifPaused == false, "LiquidStaking: Unstake functionality is paused");
        ...
        require(amount >= _minUnstake, "LiquidStaking: Invalid unstaked amount");
        ...
        require(currentSTokenBalance >= amount, "LiquidStaking: Balance should be greater than or equal to unstaked amount");
    ...
    {
        require(staker == _msgSender() || _migrationAdminContract == _msgSender(), "LiquidStaking: Unauthorised user");
        ...
        require(withdrawBalance > 0, "LiquidStaking: Withdraw amount to be greater than 0");
	 ...
	 * @param ifPause: bool value
	 *
	 * Emits a {SetFunctionPause} event
	 *
	 */
    function setFunctionPause(bool ifPause)
    public
    virtual
    override
    onlyPauserAdmin
