...
import "./interfaces/IStakeLPCoreV6.sol";
import "./libraries/TransferHelper.sol";
import "./libraries/FullMath.sol";
import "./interfaces/IWhitelistedEmissionV2.sol";

contract StakeLPCoreV15 is
	IStakeLPCoreV6,
	PausableUpgradeable,
	AccessControlUpgradeable
{
	...
	// WhitelistedEmission contract variable
		...
		)
	{
		...
		if (_totalSupplyLPTimeShare > 0) {
			...
			(, , uTokenAddresses, ) = IWhitelistedEmissionV2(
			...
			for (i = 0; i < uTokenAddresses.length; i = i.add(1)) {
				...
				_rewardPool = IUTokensV2(uTokenAddresses[i]).balanceOf(
					holderAddress
				);
				...
				rewardAmounts[rewardAmounts.length] = reward;
				rewardTokens[rewardTokens.length] = uTokenAddresses[i];
			}
		}
		...
		for (i = 0; i < otherRewardAmounts.length; i = i.add(1)) {
			rewardTokens[rewardTokens.length] = otherRewardTokens[i];
			rewardAmounts[rewardAmounts.length] = otherRewardAmounts[i];
		}
	}
		...
		)
	{
		...
		for (k = 0; k < whitelistedAddresses.length; k = k.add(1)) {
			...
			for (m = 0; m < sTokenAddresses.length; m = m.add(1)) {
				...
				isWhitelisted = ISTokensV4(sTokenAddresses[m])
					.isContractWhitelisted(whitelistedAddresses[k]);
				if (isWhitelisted) {
					rewardAmount = ISTokensV4(sTokenAddresses[m])
		...
		returns (bool success)
	{
		...
		(, , , address _lpToken) = IWhitelistedEmissionV2(_whitelistedEmission)
			.getHolderData(holderAddress);
