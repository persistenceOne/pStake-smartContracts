...
contract TokenWrapperXPRTV2 is
ITokenWrapperXPRTV2,
PausableUpgradeable,
AccessControlUpgradeable
{
    ...
    ) public virtual override whenNotPaused returns (uint256[] memory wrappedTokensArray) {
        require(to.length == amount.length, "TW11");
        require(hasRole(BRIDGE_ADMIN_ROLE, _msgSender()), "TW12");
        uint256 i;
        wrappedTokensArray = new uint256[](to.length);
        uint256 _toLength = to.length;
        for (i = 0; i < _toLength; i = i.add(1)) {
            wrappedTokensArray[i] = _generateUTokens(to[i], amount[i]);
        }
        emit GenerateUTokensInBatch(
            to,
            amount, wrappedTokensArray,
            block.timestamp
        );
    }
    ...
    function withdrawUTokens(
        ...
        unwrappedTokens = tokens.sub(_fee);
        require(_currentUTokenBalance >= unwrappedTokens, "TW16");
