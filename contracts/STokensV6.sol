...
AccessControlUpgradeable
{
    ...
    modifier onlyAdmin {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "STokens: Only admin can call this");
        _;
    }

    //modifier to check only pauser admin can call this function
    modifier onlyPauserAdmin {
        require(hasRole(PAUSER_ROLE, _msgSender()), "STokens: Only admin can call this");
        _;
    }
    ...
    function initialize(
        ...
        require(rewardRate <= _valueDivisor.mul(100), "STokens: Reward rate to be greater than or equal to value divisor");
    ...
    returns (bool)
    {
        require(_msgSender() == _liquidStakingContract, "STokens: Unauthorised user");
        _mint(to, tokens);
        return true;
    }
    ...
    returns (uint256 reward)
    {
        ...
        require(to == _msgSender() || _migrationAdminContract == _msgSender() && !isContractWhitelistedLocal, "STokens: Unauthorised user");
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused(), "STokens: Contract is in paused state");
    ...
    function setWhitelistedAddress(
        ...
        require(_msgSender() == _whitelistedPTokenEmissionContract, "STokens: Unauthorised user");
        // lpTokenERC20ContractAddress or sTokenReserveContractAddress can be address(0) but not whitelistedAddress
        require(whitelistedAddress != address(0), "STokens: Invalid whitelisted address");
