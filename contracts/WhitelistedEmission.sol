		...
		)
	{
		...
		for (j = 0; j < _holderWhitelists[holderAddress].length; j = j.add(1)) {
			...
			if (j == 0) {
				...
				uTokenAddresses = new address[](sTokenAddressesLocal.length);
				for (k = 0; k < sTokenAddressesLocal.length; k = k.add(1)) {
					uTokenAddresses[k] = ISTokensV2(sTokenAddressesLocal[k])
						.getUTokenAddress();
				}
				continue;
			}
			...
			for (
				...
				if (!isMatchingIndex) {
					...
					uTokenAddress = ISTokensV2(sTokenAddressesLocal[k])
						.getUTokenAddress();
