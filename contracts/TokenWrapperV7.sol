...
import "./interfaces/ITokenWrapperV6.sol";
import "./libraries/Bech32.sol";
import "./libraries/FullMath.sol";

contract TokenWrapperV7 is
ITokenWrapperV6,
PausableUpgradeable,
AccessControlUpgradeable
{
    ...
    address public _migrationAdminContract;

    //modifier to check only admin can call this function
    modifier onlyAdmin {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "TokenWrapper: Only admin can call this");
        _;
    }

    //modifier to check only pauser admin can call this function
    modifier onlyPauserAdmin {
        require(hasRole(PAUSER_ROLE, _msgSender()), "TokenWrapper: Only admin can call this");
        _;
    }
    ...
    returns (bool success)
    {
        ...
        require(
            ...
            "TokenWrapper: Deposit and withdraw fee should be less than or equal to value divisor"
        );
    ...
    returns (bool success)
    {
        require(minDeposit >= 1, "TokenWrapper: Required minimum deposit value");
        require(minWithdraw >= 1, "TokenWrapper: Required minimum withdraw value");
    ...
    returns (uint256 wrappedTokens)
    {
        ...
        require(to != address(0) && amount != 0, "TokenWrapper: Amount should be more than 0");
    ...
    returns (uint256 wrappedTokens)
    {
        ...
        require(hasRole(BRIDGE_ADMIN_ROLE, _msgSender()), "TokenWrapper: Unauthorised user");
    ...
    function generateUTokensInBatch(
        ...
        require(toAddresses.length == amounts.length, "TokenWrapper: Array length mismatch");
        require(hasRole(BRIDGE_ADMIN_ROLE, _msgSender()), "TokenWrapper: Unauthorised user");
    ...
    function withdrawUTokens(
        ...
        require(from == _msgSender() || _migrationAdminContract == _msgSender(), "TokenWrapper: Unauthorised user");

        // require the token amount not be less than min value
        require(tokens >= _minWithdraw, "TokenWrapper: Invalid withdraw amount");

        // require user to hold enough UTokens balance
        uint256 currentUTokenBalance = _uTokens.balanceOf(from);
        require(currentUTokenBalance >= tokens, "TokenWrapper: Balance should be greater than or equal to withdraw amount");
        ...
        require(isAddressValid == true, "TokenWrapper: Invalid cosmos address");
	 ...
	 *
	 * Emits a {SetMigrationAdminEvent} event with '_contract' set to the migration admin contract address.
	 *
	 */
    ...
    onlyAdmin
    {
        ...
        emit SetMigrationAdminEvent(migrationAdminContract);
    }
