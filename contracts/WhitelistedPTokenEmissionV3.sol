...
contract WhitelistedPTokenEmissionV3 is
	IWhitelistedPTokenEmissionV3,
	PausableUpgradeable,
	AccessControlUpgradeable
{
		...
		)
	{
		...
		if (holderAddress != address(0)) {
			...
			for (k = 0; k < sTokenAddresses.length; k = k.add(1)) {
				uTokenAddresses[k] = ISTokensV3(sTokenAddresses[k])
					.getUTokenAddress();
			}
		}
	}
		...
		for (i = 0; i < sTokenAddresses.length; i = i.add(1)) {
			...
			for (j = 0; j < whitelistedAddresses.length; j = j.add(1)) {
				...
				if (areContractsWhitelistedBools[j]) {
					(holderReward, , ) = ISTokensV3(sTokenAddresses[i])
	...
	function setWhitelistedAddress(
		...
		for (i = 0; i < sTokenAddresses.length; i = i.add(1)) {
			...
			ISTokensV3(sTokenAddresses[i]).setWhitelistedAddress(
				whitelistedAddress,
