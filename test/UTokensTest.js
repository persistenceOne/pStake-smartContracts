...
describe('UTokens', () => {
    ...
    describe("Set smart contract address", function () {
        ...
        it("Non owner can set sToken contract address: ", async function () {
            await expectRevert(utokens.setSTokenContract(stokens.address,{from: to,}), "UTokens: User not authorised to set SToken contract");
            // TEST SCENARIO END
        }, 200000);

        it("Non owner can set wrapper contract address: ", async function () {
            await expectRevert(utokens.setWrapperContract(tokenWrapper.address,{from: to,}), "UTokens: User not authorised to set wrapper contract");
            // TEST SCENARIO END
        }, 200000);

        it("Non owner can set liquidStaking contract address: ", async function () {
            await expectRevert(utokens.setLiquidStakingContract(liquidStaking.address,{from: to,}), "UTokens: User not authorised to set liquidStaking contract");
            // TEST SCENARIO END
        }, 200000);
    });
    ...
    describe("Pausable", function () {
        it('Only pauser admin can pause contracts', async function () {
            await utokens.pause({from: from,});
            let checkPause = await utokens.paused();
            expect(checkPause === true)
        });

        it('Non pauser admin cannot pause contracts', async function () {
            await expectRevert(utokens.pause({from: to,}), "UTokens: User not authorised to pause contracts");
        });
        ...
        it('Transactions could not be sent to paused contracts', async function () {
            ...
            await expectRevert(utokens.setLiquidStakingContract(liquidStaking.address,{from: from,}), "Pausable: paused");
        });
        ...
        it('Only pauser admin can unpause contracts', async function () {
            ...
            await utokens.unpause({from: from,});
            checkPause = await utokens.paused();
            expect(checkPause === false)
        });

        it('Non pauser admin cannot unpause contracts', async function () {
            await expectRevert(utokens.unpause({from: to,}), "UTokens: User not authorised to unpause contracts");
        });
    });
});
