...
describe("Liquid Staking", function () {
    ...
    it("Address and token length mismatch", async function () {
      ...
      await expectRevert(
        liquidStaking.stakeDirectInBatch(
          toAddresses,
          [new BN(100)],
          [new BN(100), new BN(150), new BN(200)],
          {
            from: bridgeAdmin,
          }
        ),
  ...
  describe("sTokens", function () {
    ...
    it("Malicious/illegitimate actor cannot mint sTokens", async function () {
      ...
      expect(balance == amount);
      await expectRevert(
        liquidStaking.stake(to, amount, { from: unknownAddress }),
        "LQ12"
      );
    });
  ...
  describe("Staking", function () {
    ...
    it("Insuffcient balance for account", async function () {
      let generate = await tokenWrapper.generateUTokens(to, amt, {
        from: bridgeAdmin,
      });
      let balance = await utokens.balanceOf(to);
      expectEvent(generate, "GenerateUTokens", {
  ...
  describe("Set Reward Rate", function () {
    it("Non-Owner cannot call setRewardRate()", async function () {
      await expectRevert(
        stokens.setRewardRate(rate, { from: unknownAddress }),
        "ST2"
      );
    }, 200000);
  ...
  describe("Calculate Reward", function () {
    it("Only staker can initiate their own rewards calculation", async function () {
      expectRevert(
        stokens.calculateRewards(to, { from: unknownAddress }),
        "ST5"
      );
    }, 200000);
  ...
  describe("Transfer", function () {
    it("Transfer amount exceeds balance", async function () {
      await expectRevert(
        stokens.transfer(unknownAddress, amount, { from: to }),
        "revert"
      );
    }, 200000);
    ...
    it("Transfer", async function () {
      ...
      let transferTokens = await stokens.transfer(unknownAddress, val, {
        from: to,
      });
  ...
  describe("UnStaking", function () {
    ...
    it("Non-staker cannot withdraw", async function () {
      await expectRevert(
        liquidStaking.unStake(to, amount, { from: unknownAddress }),
        "LQ15"
      );
    }, 200000);
    ...
    it("Current sToken balance should be greater than unstaked amount", async function () {
      ...
      expect(balance >= amt);
      await expectRevert(
        liquidStaking.unStake(to, amount, { from: to }),
        "LQ19"
      );
    }, 200000);
    ...
    it("Get unbonding tokens", async function () {
      ...
      let unbondTokens = await liquidStaking.getTotalUnbondedTokens(to, {
        from: to,
      });
  ...
  describe("Withdraw", function () {
    describe("Withdraw uTokens", function () {
      it("Number of utokens should be greater than 0", async function () {
        ...
        await expectRevert(
          tokenWrapper.withdrawUTokens(to, _val, toAtomAddress, { from: to }),
          "TW14"
        );
      }, 200000);
      ...
      it("Current uToken balance should be greater than staked amount", async function () {
        ...
        await expectRevert(
          tokenWrapper.withdrawUTokens(to, amount, toAtomAddress, { from: to }),
          "TW16"
        );
      }, 200000);
      ...
      it("Only staker can withdraw", async function () {
        ...
        await expectRevert(
          tokenWrapper.withdrawUTokens(to, amt, toAtomAddress, {
            from: unknownAddress,
          }),
          "TW17"
        );
    ...
    describe("Withdraw unStaked tokens", function () {
      it("Only staker can withdraw", async function () {
        await expectRevert(
          liquidStaking.withdrawUnstakedTokens(to, { from: unknownAddress }),
          "LQ20"
        );
      }, 200000);
    });
  ...
  describe("Pausable", function () {
    ...
    it("Transactions could not be sent to paused contracts", async function () {
      ...
      await expectRevert(
        liquidStaking.stake(to, amt, { from: to }),
        "Pausable: paused"
      );
    });
